<?php
	...
	infra_require('*cart/filters.inc.php');
	$search=$_GET['search'];
	$conf=infra_config();
	$data=cat_search($search);	
	$ans=infra_cache(array($conf['cart']['dir'], $conf['cart']['prod'], $conf['cart']['1c']),'filters',function($search,$l) use(&$data){
		...
		$count=sizeof($data['list']); //количество позиций
		foreach($data['list'] as &$pos){
			...
			if(!$pos['more'])$pos['more']=array(); //Если нет характеристик
			...
			$p=array(
				'more'=>$pos['more'], ///Например more=array('Разрешение'=>'22342','Ночная съёмка'=>'Есть')
				'params'=>array(
			...
			if($pos['Цена розничная']){
				$p['params']['Цена розничная']=(int)$pos['Цена розничная'];//Дробей в php нет, всё что после точки удаляется	
			}
			
			$p['params']['Яндекс Маркет']=$pos['market']?'Размещён':'Не размещён';	
		...
		foreach($params as $k=>$p){
			if($conf['catalog']['1c']){
				$showhard=in_array($k,array('Наличие на складе','Синхронизация','Яндекс Маркет'));
			}else{
				$showhard=array();
			}
			...
			$opt['no']=$count-$opt['yes']; //из общего количества вычесть количество с yes
			$opt['name']=$k;
			$params[$k]=$opt;
			//===
		}
