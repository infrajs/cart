...
{COUPONCHECK:}
	...
	{CART:}
		...
		<p>
			В вашей <a href="/cart/orders/active/list">корзине</a> <b>{~length(data.list.0.basket)|:0}</b> {~words(~length(data.list.0.basket)|:0,:позиция,:позиции,:позиций)}.
		</p>
		...
		{stinfo:}
			<tr class="{data.rules.rules[~key].notice}">
				...
				<td>{data.meta.rules[status].title}</td>
				<td>{~length(basket)}</td>
				<td>{~cost(sum)}{:model.unit}</td>
	...
	{ORDERS:}
		...
		<div style="margin-top:10px">
			<a href="/cart/orders/active/list" style="text-decoration:none" class="btn btn-success">Заказ ({data.order.count} {~words(data.order.count,:позиция,:позиции,:позиций)})</a>
		</div>
...
{listcrumb:}
	...
	<ol class="breadcrumb">
		...
		<li class="breadcrumb-item"><a href="/cart/{data.place}/{data.order.id|:active}">Оформление заказа {data.order.id}</a></li>
	</ol>
{ordercrumb:}
	{:usersync}
	<ol class="breadcrumb">
...
{PRINT:}
	<ol class="breadcrumb noprint">
		...
		<li class="breadcrumb-item"><a class="{crumb.parent.parent.name=:admin?:text-danger}" href="/{crumb.parent}">Заказ {crumb.parent.name=:active??crumb.parent.name}</a></li>
