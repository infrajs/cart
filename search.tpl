{root:}
	...
	<script type="text/javascript">
		infra.wait(infrajs,'onshow',function(){
			...
			inp.val(val);//В сессии не надо сохранять
		});
	</script>
	{searchgood:}
		<style>
		...
		{nalichie:}<span class="label {Наличие на складе=:В наличии?:label-primary?(Наличие на складе=:Распродажа?:label-success?(Наличие на складе=:На заказ?:label-info?:label-default))}">{Наличие на складе}</span>{market?:market}<br>
{market:}
<img src="infra/plugins/imager/imager.php?src=/icons/yandex.png&h=24&w=24&crop=1" alt=" яндекс-маркет" title="Товар размещен на Яндекс Маркете" />
